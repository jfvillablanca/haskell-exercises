...
-}
lowerAndGreater n list = compare n 0 0 list
  where
    compare num lt gt [] = show num ++ " is greater than " ++ show lt ++ " elements and lower than " ++ show gt ++ " elements"
    compare num lt gt (x : xs)
        | x < num = compare num (lt + 1) gt xs
        | x > num = compare num lt (gt + 1) xs
        | otherwise = compare num lt gt xs
